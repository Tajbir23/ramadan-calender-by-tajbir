...
function createLanguagePassthroughModule(file) {
  ...
  const lines = [`import ${importName} from "refractor/lang/${file}";`, `export default ${importName}`, ''];
...
function createSupportedLanguagesArray(files) {
  ...
  lines.push(`\n];\n`);
  fs.writeFile(path.join(__dirname, `../src/languages/prism/supported-languages.js`), lines.join(''), err => {
    if (err) {
      throw err;
    }
  });
