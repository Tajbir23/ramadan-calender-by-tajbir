...
let directories = ['../node_modules/prismjs/themes', '../node_modules/prism-themes/themes'];
...
function updateDocs(files) {
  ...
  const onlyNonMinifiedCSS = onlyCSSFiles.filter(file => !file.includes('.min'));
  const availableStyleNames = onlyNonMinifiedCSS.map(file => getSimpleFilename(file));
  ...
  fs.writeFile(path.join(__dirname, '../AVAILABLE_STYLES_PRISM.MD'), styleMD, () => {});
  fs.writeFile(path.join(__dirname, '../src/styles/prism/index.js'), defaultExports.join(''), () => {});
  const demoStylesArray = `${autogenMessage}export default [${availableStyleNames.sort().map(style => `\n  '${style}'`)}\n];`;
  fs.writeFile(path.join(__dirname, '../demo/styles/prism.js'), demoStylesArray, err => {
    if (err) {
      throw err;
    }
  });
...
function createJavascriptStyleSheet(file, directory) {
  ...
  fs.readFile(path.join(__dirname, `${directory}/${file}`), 'utf-8', (err, data) => {
        ...
        const style = rule.selectors.reduce((selectors, selector) => {
          const selectorObject = rule.declarations.reduce((declarations, declaration) => {
            if (declaration.type === 'declaration' && declaration.property) {
              ...
              const key = camelCaseDeclarationProp.includes('moz') || camelCaseDeclarationProp.includes('webkit') || camelCaseDeclarationProp[0] === 'o' && !camelCaseDeclarationProp.includes('overflow') ? `${camelCaseDeclarationProp.substring(0, 1).toUpperCase()}${camelCaseDeclarationProp.substring(1)}` : camelCaseDeclarationProp;
    ...
    fs.writeFile(path.join(__dirname, `../src/styles/prism/${fileWithoutCSS}.js`), `export default ${JSON.stringify(javacriptStylesheet, null, 4)}`, () => {});
  });
}