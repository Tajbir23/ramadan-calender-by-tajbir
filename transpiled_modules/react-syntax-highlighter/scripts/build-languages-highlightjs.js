...
function createAsyncLanguageLoadersIndex(files) {
  let lines = [`import createLanguageAsyncLoader from "./create-language-async-loader"`, `export default {`];
...
function createSupportedLanguagesArray(files) {
  ...
  lines.push(`\n];\n`);
  fs.writeFile(path.join(__dirname, `../src/languages/hljs/supported-languages.js`), lines.join(''), err => {
    if (err) {
      throw err;
    }
  });
...
function createLanguagePassthroughModule(file) {
  const fileWithoutJS = file.split('.js')[0];
  const importName = makeImportName(fileWithoutJS);
  const lines = [`import ${importName} from "highlight.js/lib/languages/${fileWithoutJS}"`, `export default ${importName}`, ''];
