...
fs.readdir(path.join(__dirname, '../node_modules/highlight.js/styles'), (err, files) => {
  ...
  const availableStyleNames = onlyCSSFiles.map(file => file.split('.css')[0] === 'default' ? 'default-style' : file.split('.css')[0]);
  ...
  fs.writeFile(path.join(__dirname, '../demo/styles/hljs.js'), demoStylesArray, err => {
    if (err) {
      throw err;
    }
  });
...
function createJavascriptStyleSheet(file) {
  ...
  fs.readFile(path.join(__dirname, `../node_modules/highlight.js/styles/${file}`), 'utf-8', (err, data) => {
    ...
    fs.writeFile(path.join(__dirname, `../src/styles/hljs/${fileWithoutCSS}.js`), `export default ${JSON.stringify(javacriptStylesheet, null, 4)}`, err => {
      if (err) {
        throw err;
      }
    });
